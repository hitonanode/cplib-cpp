...
data:
  ...
  bundledCode: "#line 1 \"flow/test/submodular_opt.yuki1479.test.cpp\"\n#define PROBLEM\
    ...
    \ total_cost, sol};\n    }\n};\n#line 3 \"flow/test/submodular_opt.yuki1479.test.cpp\"\
    \n\n#include <iostream>\n#line 6 \"flow/test/submodular_opt.yuki1479.test.cpp\"\
    \n#include <set>\n#line 8 \"flow/test/submodular_opt.yuki1479.test.cpp\"\nusing\
    \ namespace std;\n\nint main() {\n    int H, W;\n    cin >> H >> W;\n\n    map<int,\
    \ vector<pair<int, int>>> val2grid;\n    for (int i = 0; i < H; ++i) {\n     \
    \   for (int j = 0; j < W; ++j) {\n            int a;\n            cin >> a;\n\
    \            if (a) val2grid[a].emplace_back(i, j);\n        }\n    }\n\n    int\
    \ ret = 0;\n    for (const auto &[val, points] : val2grid) {\n        SubmodularOptimizationViaGraphCut<pair<char,\
    \ int>> so;\n\n        set<int> xs, ys;\n        for (auto [x, y] : points) {\n\
    \            xs.insert(x);\n            ys.insert(y);\n            so.Impose({'r',\
    \ x}, false, {'c', y}, false, 1 << 20);\n        }\n        for (int x : xs) so.Impose({'r',\
  ...
  verifiedWith: []
documentation_of: flow/test/submodular_opt.yuki1479.test.cpp
layout: document
redirect_from:
- /verify/flow/test/submodular_opt.yuki1479.test.cpp
- /verify/flow/test/submodular_opt.yuki1479.test.cpp.html
