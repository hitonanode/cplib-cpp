...
data:
  ...
  bundledCode: "#line 1 \"formal_power_series/test/fps_exp_modintruntime.test.cpp\"\
    ...
    \ i < N; i++) cin >> A[i];\n    A.shrink();\n    auto ret = A.exp(N);\n    for\
    \ (int i = 0; i < N; i++) printf(\"%d \", ret.coeff(i).val);\n}\n"
...
layout: document
redirect_from:
- /verify/formal_power_series/test/fps_exp_modintruntime.test.cpp
- /verify/formal_power_series/test/fps_exp_modintruntime.test.cpp.html
