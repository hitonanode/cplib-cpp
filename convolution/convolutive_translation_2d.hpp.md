...
data:
  ...
  bundledCode: "#line 1 \"convolution/convolutive_translation_2d.hpp\"\n#include <algorithm>\n\
    ...
    \ convolutive_translation_2d(\n    const std::vector<std::vector<MODINT>> &initial_dist,\
    ...
    \ (p.first.second <= -W or p.first.second >= W) continue;\n        xlo = std::max(xlo,\
    \ -p.first.first), xhi = std::max(xhi, p.first.first);\n        ylo = std::max(ylo,\
    \ -p.first.second), yhi = std::max(yhi, p.first.second);\n        t2c_1d.emplace_back(p);\n\
    \    }\n    const int WW = W + ylo + yhi;\n    std::vector<MODINT> dp((H - 1)\
    \ * WW + W);\n    for (int i = 0; i < H; i++)\n    {\n        std::copy(initial_dist[i].begin(),\
    \ initial_dist[i].end(), dp.begin() + i * WW);\n    }\n\n    int tlo = 0, thi\
    \ = 0;\n    for (auto p : t2c_1d)\n    {\n        int t = p.first.first * WW +\
  ...
  requiredBy: []
  timestamp: '2020-08-25 07:05:42+09:00'
