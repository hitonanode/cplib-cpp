...
Point2d<Float>
circumcenter(const Point2d<Float> &A, const Point2d<Float> &B, const Point2d<Float> &C) {
    std::complex<Float> a = (C - A).to_complex(), b = (B - A).to_complex();
    return A + Point2d<Float>(a * b * std::conj(a - b) / (std::conj(a) * b - a * std::conj(b)));
}
template <typename Float>
Point3d<Float>
circumcenter(const Point3d<Float> &A, const Point3d<Float> &B, const Point3d<Float> &C) {
