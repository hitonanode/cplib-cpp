...
data:
  ...
  code: "#pragma once\n#include \"acl_lazysegtree.hpp\"\n\n// CUT begin\ntemplate\
    ...
    \ k, F f) const override {\n        const_cast<segtree_beats *>(this)->d[k] =\
    \ mapping(f, Base::d[k]);\n        if (k < Base::size) {\n            const_cast<segtree_beats\
    \ *>(this)->lz[k] = composition(f, Base::lz[k]);\n            if (Base::d[k].fail)\
    ...
    \ <typename Num> inline Num second_highest(Num a, Num a2, Num b, Num b2) noexcept\
    \ { // a > a2, b > b2\n    return a == b ? std::max(a2, b2) : a2 >= b ? a2 : b2\
    \ >= a ? b2 : std::min(a, b);\n}\n\nusing BNum = long long;\nconstexpr BNum BINF\
    \ = 1LL << 61;\n\nstruct S {\n    BNum lo, hi, lo2, hi2, sum;\n    unsigned sz,\
    \ nlo, nhi;\n    bool fail;\n    S() : lo(BINF), hi(-BINF), lo2(BINF), hi2(-BINF),\
    \ sum(0), sz(0), nlo(0), nhi(0), fail(0) {}\n    S(BNum x, unsigned sz_ = 1)\n\
    ...
    \    ret.lo2 = second_lowest(l.lo, l.lo2, r.lo, r.lo2), ret.hi2 = second_highest(l.hi,\
