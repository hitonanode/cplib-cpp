...
data:
  ...
  bundledCode: "#line 2 \"linear_algebra_matrix/linalg_modint.hpp\"\n#include <algorithm>\n\
    ...
    \ <typename T> std::pair<std::vector<T>, std::vector<std::vector<T>>> system_of_linear_equations(matrix<T>\
    \ A, std::vector<T> b) {\n    int H = A.H, W = A.W;\n    matrix<T> M(H, W + 1);\n\
  ...
  code: "#pragma once\n#include \"linear_algebra_matrix/linalg_modint.hpp\"\n#include\
    ...
    \ O(HW(H + W))\ntemplate <typename T> std::pair<std::vector<T>, std::vector<std::vector<T>>>\
