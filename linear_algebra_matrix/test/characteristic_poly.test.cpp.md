...
data:
  ...
  bundledCode: "#line 1 \"linear_algebra_matrix/test/characteristic_poly.test.cpp\"\
    ...
    \ T_> static auto check(T_ *) -> decltype(T_::id(), std::true_type());\n    template\
    \ <class T_> static auto check(...) -> std::false_type;\n};\ntemplate <class T_>\
    \ struct has_id_method : decltype(has_id_method_impl::check<T_>(nullptr)) {};\n\
    ...
    \ + 1); // p[i + 1] = (Characteristic polynomial of i-th leading principal minor)\n\
    \    p[0] = {1};\n    for (int i = 0; i < N; i++) {\n        p[i + 1].assign(i\
