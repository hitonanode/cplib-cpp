...
template <class MODINT, class MATRIX, typename std::enable_if<std::is_same<MATRIX, toeplitz<MODINT>>::value>::type* = nullptr>
toeplitz<MODINT> gen_random_matrix() {
    ...
    vw[0] = vh[0];
    return toeplitz<MODINT>(vh, vw);
}

template <class MODINT, class MATRIX, typename std::enable_if<std::is_same<MATRIX, square_toeplitz<MODINT>>::value>::type* = nullptr>
square_toeplitz<MODINT> gen_random_matrix() {
    int N = uniform_int_distribution<int>(1, 100)(mt);
    auto v = gen_random_vector<MODINT>(N * 2 - 1);
    return square_toeplitz<MODINT>(v);
}
...
template <typename MATRIX, typename mint> void blackbox_mat_checker(int n_try) {
    for (int i = 0; i < n_try; i++) {
        ...
        vector<vector<mint>> Mvv = M.dense();
...
int main() {
    ...
    blackbox_mat_checker<toeplitz<mint>, mint>(1000);
    blackbox_mat_checker<square_toeplitz<mint>, mint>(1000);
