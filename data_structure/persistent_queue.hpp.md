...
data:
  ...
  bundledCode: "#line 2 \"data_structure/persistent_queue.hpp\"\n#include <array>\n\
    ...
    \ on each node of tree\n    std::vector<std::array<int, D + 1>> par; // binary-lifted\
    \ parents\n    std::vector<int> back_id;                // back_id[t] = leaf id\
    \ of the tree at time t\n    std::vector<int> size;                   // size[t]\
    \ = size of the queue at time t\n\n    persistent_queue() : now(0), data(1), par(1),\
  ...
  code: "#pragma once\n#include <array>\n#include <cassert>\n#include <utility>\n\
    ...
    \ D + 1>> par; // binary-lifted parents\n    std::vector<int> back_id;       \
    \         // back_id[t] = leaf id of the tree at time t\n    std::vector<int>\
    \ size;                   // size[t] = size of the queue at time t\n\n    persistent_queue()\
  ...
  requiredBy: []
  timestamp: '2026-09-05 15:18:12+09:00'
